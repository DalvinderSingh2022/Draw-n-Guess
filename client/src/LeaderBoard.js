  ...
  return (
    <div className="md:w-1/2 container mx-auto h-96 flex flex-col">
      <div className="heading primary">
        ...
        {room && (
          <span>
            {room.players.findIndex((play) => play.id === socket.id) + 1}/
            {room.players.length}
          </span>
        )}
      </div>
        ...
        {room ? (
          room.players
            ...
            .map((player, index) => (
              <div
                key={player.id}
                className={`${player.id !== socket.id ? "primary" : "secondary"} text-xl font-bold  px-4 py-2 rounded-xl flex justify-between`}
              >
                <span>
                  {index + 1}. {player.name}
                  {room.players[room.turnIndex - 1]?.id === player.id
                    ? " (Drawing)"
                    : ""}
                </span>
