...
const Home = () => {
  ...
  const hostRoom = (event) => {
    event.preventDefault();
    const { maxPlayers, maxRounds, drawTime, roomType, roomName } =
      event.target;
    const { name: userName, image } = user;
  ...
  const handleChange = async (e) => {
    ...
    Cookies.set(
      "drawnguess",
      JSON.stringify({ ...user, [name]: e.target.value }),
      { expires: 7 },
    );
  };
  ...
  return (
    <div className="max-w-2xl m-auto py-8 px-2">
      <div className="flex flex-col items-center">
        <div className="relative">
          <label
            htmlFor="image"
            title="upload Image"
            className="button primary rounded-full p-2 absolute -right-1"
          >
            <RiImageAddLine />
          </label>
          <button
            title="random"
            className="button primary rounded-full p-2 absolute -right-5 top-10"
            onClick={() =>
              handleImage(
      ...
      <div className="m-auto mt-10 container">
        ...
        <form
          onSubmit={(event) => joinRoom(roomId, event)}
        ...
        >
          ...
          />
          <button className="button primary px-6 py-2 rounded-r-xl">
            Join
          </button>
        </form>
      </div>
            ...
            publicRooms.length ? (
              publicRooms.map((room) => (
                <div
                  ...
                  key={room.id}
                  className="cursor-pointer primary text-xl font-bold px-5 py-2 rounded-xl flex flex-wrap justify-between items-center"
                >
      ...
      {create && (
        <div className="fixed_container justify-center bg-blue-200/50 items-center">
          <form
            onSubmit={hostRoom}
            className="mx-4 bg-white w-full max-w-xl border-b-4 border-orange-400 rounded-2xl animate-popin"
          >
            <div className="heading secondary items-center">
              <div>Create Room</div>
              <button
                onClick={() => setCreate(false)}
                className="button primary px-5 py-1 rounded-xl -mr-3"
              >
                Back
              </button>
            </div>
            ...
            <div className="flex flex-col items-center gap-2 py-4 px-8">
              ...
              <label
                ...
                className="w-full flex justify-between items-center"
              >
                <span className="text-black/90 text-2xl font-bold">
                  DrawTime
                </span>
                <input
                  required
              ...
              <button className="button secondary rounded-xl px-6 py-2">
                host
              </button>
            </div>
          </form>
